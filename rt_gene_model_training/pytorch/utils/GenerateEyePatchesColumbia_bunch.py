...
if __name__ == "__main__":
    script_desc = open('GenerateEyePatchesColumbia.py')
    script_main = script_desc.read()

    dataset_location = '/media/grant/Dataset_SSD/Columbia_raw_image'
    sample_directory_list = os.listdir(dataset_location)
    extracted_patch_dir = 'Processed_eyes'
    counter = 0

    for each_dir in sample_directory_list:
        print('process sample {} out of {}'.format(counter, len(sample_directory_list)))
        whole_face_dir = os.path.join(dataset_location, each_dir)
        ...
        sys.argv = ["GenerateEyePatchesRTGENEDataset.py", whole_face_dir, '--output_path', save_dir]
        exec(script_main)
        counter += 1
